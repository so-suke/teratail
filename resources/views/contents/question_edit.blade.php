...
<div class="container">
  <form action="{{ route('edit_question') }}" method="post">
    ...
    {{-- v-model=titleを消した --}}
    <input class="form-control" value="{{ $question->title }}" type="text" name="question_title" placeholder="タイトル：わからないこと、解決したいことを10文字以上で書いてください" required>
    ...
    <div class="d-flex flex-column">
      <div class="d-flex">
        <div class="d-flex flex-column">
          ...
          <textarea class="md-height" name="md_content" ref="mdInpPlace" @input="convert" cols="30" rows="10" placeholder="分からないこと、解決したいことの詳細を30～10000文字で入力してください。" required>{{ $question->md_content }}</textarea>
        </div>
      ...
      <button class="btn btn-primary col-1" type="submit">更新する</button>
    </div>
...
<script src="{{ asset('/js/contents/question_edit.js') }}"></script>
@endsection
